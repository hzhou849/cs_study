...
'''
02/02/2018	- Remove on/off for latch mode
...
class GPIO_Tester (Frame):
	...
	def initUI(self,main):
		# Set the app title
		self.master.title("GPIO Test Application v1.2")
	...
	def GPIO_Labels(self):
		'''
		Refer to RE000122665 for GPIO details
		'''
		gpio0Label = Label(self, text="GPIO 0-SysPau", background=backgroundColor)
		gpio0Label.grid(row=1, column=0)
		gpio1Label = Label(self, text="GPIO 1-SysAct", background=backgroundColor) 
		gpio1Label.grid(row=2, column=0)
		gpio2Label = Label(self, text="GPIO 2-SysPau_Latch", background=backgroundColor)
		gpio2Label.grid(row=3, column=0)
		gpio4Label = Label(self, text="GPIO 4-SysAct_LED", background=backgroundColor)
		gpio4Label.grid(row=4, column=0)
		gpio6Label = Label(self, text="GPIO 6-SysPau_LED", background=backgroundColor)
		gpio6Label.grid(row=5, column=0)
		gpio7Label = Label(self, text="GPIO 7-SysPau_Latch_CLR_L", background=backgroundColor)
		gpio7Label.grid(row=6, column=0)
		gpio8Label = Label(self, text="GPIO 8-SysAct_DB_L", background=backgroundColor)
		gpio8Label.grid(row=7, column=0)
		gpio9Label = Label(self, text="GPIO 9-SysPau_DB", background=backgroundColor)
	...
	def LatchResetComponents_Init(self):
		...
		self.LatchRadioOn = Radiobutton(self, text="On",relief=SUNKEN, \
										...
										background=backgroundColor, variable=self.v, value=1)
		self.LatchRadioOn.grid(row=8, column=3, sticky=W)

		self.LatchRadioOff = Radiobutton(self, text="Off",relief=SUNKEN,\
										command=lambda: self.LatchResetButtonPress("latchOff"), \
										background=backgroundColor, variable=self.v, value=2)
		self.LatchRadioOff.grid(row=8, column=4, sticky=W)
		self.LatchRadioOn.invoke()
