...
public:
    ...
    Ol_string operator-() const;                // make lowercase of string
    Ol_string operator+(const Ol_string &rhs) const;   // concat. binary op requires input
    bool operator==(const Ol_string &rhs) const;
...
// Equality
bool Ol_string::operator==(const Ol_string &rhs) const {
    return (std::strcmp(str, rhs.str) == 0); // strcmp retuns 0 if equal, if 0 == 0--> return 1, true
}

// Lower case
Ol_string Ol_string::operator-() const {
    // create a temp buffer in heap to copy the string into
    char *buff = new char[std::strlen(str) + 1];
    // copy the string into the buffer
    std::strcpy(buff, str);

    // iterate through the buffer and make each lower case
    for (size_t i=0; i < std::strlen(buff); i++) {  
        buff[i] = std::tolower(buff[i]); // make lower case
    }
...
// Concatenate
Ol_string Ol_string::operator+(const Ol_string &rhs) const {
    char *buff = new char [std::strlen(str) + std::strlen(rhs.str) +1];
    std::strcpy(buff, str);     //copy first part of string into buff
    std::strcat(buff, rhs.str); // copy the second string into buff
    Ol_string temp {buff};
    delete [] buff;
    return temp;
}
